...
class PHPEntitiesParser {
  	...
  	public function LocationParser($str){
  		...
  		foreach($AllPotentialLocations as $PotentialLocation){
  			try{
	  			...
	  			if(count($results)>0 && strpos(strtolower($results[0]['formatted_address']), $PotentialLocation)>-1){
      ...
      $vocab=["date","today","yesterday","holiday","ticket","tickets","match","matched","homes","stoves","refrigerators","home","kitchen","stove","sink","refrigerator","plates","dishes","fork","spoon","etc.","bedroom","bed","lamp","fan","sheets","etc.","living room","sofa","chairs","piano"];
  		$stopwords=array_merge($stopwords,$this->EmailsParser(strtolower($str)),$vocab);
