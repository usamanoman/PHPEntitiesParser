...
class DateExtraction
{
  ...
  function parse_date($text)
  {
    ...
    if($ts!=false){
      if(is_int(key($ts))){
        return ["next " . $aStates[0]['word']=>date('Y-m-d H:i:s', $ts[key($ts)])];
      }else{
  ...
  function get_time($aWords)
  {
    ...
    if(isset($data['day']) && isset($data['month']) && isset($data['year']) && isset($data['hour']) && isset($data['minute'])){
      return [$data['year'].'-'.$data['month'].'-'.$data['day'].' '.$data['hour'].':'.$data['minute'].':00'=>mktime($data['hour'], $data['minute'], 0, $data['month'], $data['day'], $data['year'])];
    }
    if(isset($data['day']) && isset($data['month']) && isset($data['year']) && isset($data['hour'])){
      return [$data['year'].'-'.$data['month'].'-'.$data['day'].' '.$data['hour'].':00:00'=>mktime($data['hour'], 0, 0, $data['month'], $data['day'], $data['year'])];
    }
    else if(isset($data['day']) && isset($data['month']) && isset($data['hour']) && isset($data['minute'])){
      return [$this->predict_year($data['month']).'-'.$data['month'].'-'.$data['day'].' '.$data['hour'].':'.$data['minute'].':00'=>mktime($data['hour'], $data['minute'], 0, $data['month'], $data['day'], $this->predict_year($data['month']))];
    }
    else if(isset($data['day']) && isset($data['month']) && isset($data['hour'])){
      return [$this->predict_year($data['month']).'-'.$data['month'].'-'.$data['day'].' '.$data['hour'].':00:00'=>mktime($data['hour'], 0, 0, $data['month'], $data['day'], $this->predict_year($data['month']))];
    }
    else if(isset($data['day']) && isset($data['month'])){
      //default time if not defined: 00:00
      return [$this->predict_year($data['month']).'-'.$data['month'].'-'.$data['day'].' 00:00:00'=>mktime(0, 0, 0, $data['month'], $data['day'], $this->predict_year($data['month']))];
    }
    else if($data['day_of_week'] != 0){
      return [$data['day_of_week']=>$data['day_of_week']];
    }
